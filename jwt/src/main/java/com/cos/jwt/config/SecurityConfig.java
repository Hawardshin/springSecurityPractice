...
public class SecurityConfig {
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		http.authorizeHttpRequests(authorize ->
			authorize
				.requestMatchers("/api/v1/user/**").hasAnyRole("USER", "ADMIN","MANAGER") //인증은 물론이고, USER, ADMIN 권한이 있어야 들어갈 수 있는 주소
				.requestMatchers("/api/v1/manager/**").hasAnyRole("ADMIN","MANAGER") //인증은 물론이고, USER, ADMIN 권한이 있어야 들어갈 수 있는 주소
				.requestMatchers("/api/v1/admin/**").hasAnyRole("ADMIN") //인증은 물론이고, ADMIN 권한이 있어야 들어갈 수 있는 주소
